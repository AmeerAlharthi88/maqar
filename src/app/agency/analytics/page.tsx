  ...
  return (
    <AgencyDashboardShell titleAr="تحليلات الوكالة">
      <div className="px-4 py-4 space-y-4" dir="rtl">
        ...
        {a.topAreas.length > 0 && (
          <div>
            <h2 className="text-sm font-bold text-[#102A43] mb-3">أكثر المناطق مشاهدةً</h2>
            <div className="space-y-2">
              {a.topAreas.map((area, idx) => {
                const maxViews = a.topAreas[0].views;
                const pct = Math.round((area.views / maxViews) * 100);
                return (
                  <div key={area.areaAr} className="bg-white rounded-2xl border border-[#E2E8F0] px-4 py-3">
                    <div className="flex items-center justify-between mb-1.5">
                      <p className="text-sm font-bold text-[#102A43]">{area.areaAr}</p>
                      <p className="text-xs text-[#627D98]">{area.views.toLocaleString("ar-OM")} مشاهدة</p>
                    </div>
        ...
        {a.topAgents.length > 0 && (
          <div>
            <h2 className="text-sm font-bold text-[#102A43] mb-3">أداء الوسطاء</h2>
            <div className="space-y-2">
              {a.topAgents.map((agent, idx) => (
                ...
                >
                  ...
                  <div className="flex gap-3 text-xs flex-shrink-0">
                    ...
                    <span className="text-[#0A3C36] font-semibold">{agent.sales} صفقة</span>
                  </div>
                </div>
              ))}
            </div>
