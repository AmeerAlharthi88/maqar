...
function AgentPreviewCard({ agent }: AgentPreviewCardProps) {
  return (
    <Link
      ...
      className="flex-shrink-0 w-36 bg-white rounded-2xl border border-[#E2E8F0] p-3 flex flex-col items-center gap-2 text-center shadow-[0_2px_8px_0_rgb(10_60_54/0.06)] hover:shadow-[0_6px_20px_0_rgb(10_60_54/0.10)] transition-shadow"
    >
      <Avatar name={agent.nameAr} size="lg" />
      <div className="w-full min-w-0">
        <p className="text-xs font-bold text-[#102A43] truncate">{agent.nameAr}</p>
        {agent.agency && (
          <p className="text-[10px] text-[#627D98] truncate">{agent.agency.nameAr}</p>
        )}
      </div>
      {agent.isVerified && (
