  ...
  return (
    <div
      className={cn(
        ...
        "shadow-[0_2px_8px_0_rgb(30_30_30/0.06)]",
        className
      )}
    >
      <div className="flex items-start justify-between mb-3">
        <div>
          <h3 className="text-base font-bold text-[#1E1E1E]">{stat.areaAr}</h3>
        ...
        <span
          className={cn(
            ...
            isPositive ? "bg-[#EDF4ED] text-[#5B8C5A]" : "bg-[#FEF0EE] text-[#C0392B]"
          )}
        >
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="flex flex-col gap-0.5">
          <p className="text-xs text-[#7A6B5E]">متوسط سعر البيع</p>
          <p className="text-sm font-bold text-[#C65D3B]">{formatOMR(stat.avgSalePrice, { compact: true })}</p>
        </div>
        <div className="flex flex-col gap-0.5">
          <p className="text-xs text-[#7A6B5E]">متوسط الإيجار</p>
      ...
      <div className="mt-3 pt-3 border-t border-[#F0EBE3] flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5">
          <div className="w-16 h-1.5 rounded-full bg-[#F0EBE3] overflow-hidden">
            <div
              className="h-full rounded-full bg-[#C65D3B]"
              style={{ width: `${stat.demandScore}%` }}
            />
          </div>
          <span className="text-xs text-[#7A6B5E]">{toArabicNumerals(stat.demandScore)}%</span>
        </div>
      </div>
    </div>
  );
