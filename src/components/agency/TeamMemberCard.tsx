...
export function TeamMemberCard({
  ...
  return (
    <div className="bg-white rounded-2xl border border-[#E2E8F0] px-4 py-4 flex items-center gap-3" dir="rtl">
      {/* Avatar */}
      <div className="w-11 h-11 rounded-full bg-[#0A3C36]/15 flex items-center justify-center flex-shrink-0">
        {member.avatar ? (
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5 flex-wrap">
          <p className="text-sm font-bold text-[#102A43] truncate">{member.nameAr}</p>
          {member.isVerified && (
        ...
        <div className="flex items-center gap-2 mt-0.5">
          <StatusBadge
            ...
            size="xs"
          />
          <span className="text-[10px] text-[#627D98]">{member.activeListings} إعلان نشط</span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-1.5">
        <a
          href={callHref}
          className="w-8 h-8 rounded-xl bg-[#F0F4F8] flex items-center justify-center"
          aria-label="اتصال"
        >
        ...
        {isOwner && member.role !== "agency_admin" && (
          <>
            ...
            <button
              ...
              className="w-8 h-8 rounded-xl bg-[#FEF0EE] flex items-center justify-center"
              aria-label="إزالة"
            >
