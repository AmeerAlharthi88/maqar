    ...
    <div className="bg-white rounded-2xl border border-[#E2E8F0] px-4 py-4" dir={isAr ? "rtl" : "ltr"}>
      ...
      <div className="flex items-start justify-between gap-2 mb-3">
        <div className="flex-1 min-w-0">
          <p className="text-sm font-bold text-[#102A43] line-clamp-2">{displayMeta(flag.listingTitleAr, isAr)}</p>
          <p className="text-xs text-[#627D98] mt-0.5">{displayMeta(flag.agentNameAr, isAr)} · {displayMeta(flag.areaAr, isAr)}</p>
          <p className="text-[10px] text-[#627D98] mt-0.5">
            {new Date(flag.submittedAt).toLocaleDateString(numLocale, { year: "numeric", month: "long", day: "numeric" })}
          </p>
        </div>
        <div className="flex flex-col items-end gap-1 flex-shrink-0">
          <StatusBadge label={isAr ? AML_STATUS_AR[flag.status] : AML_STATUS_EN[flag.status]} variant={STATUS_VARIANT[flag.status]} size="xs" />
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        <div className="bg-[#F8F9FA] rounded-xl py-2 text-center">
          <p className="text-xs font-bold text-[#0A3C36]">
            {flag.listingPrice.toLocaleString(numLocale)}
          </p>
          <p className="text-[10px] text-[#627D98]">{bi(isAr, "سعر الإعلان", "Listing price")}</p>
        </div>
        <div className="bg-[#F8F9FA] rounded-xl py-2 text-center">
          <p className="text-xs font-bold text-[#102A43]">
            {flag.marketAvgPrice.toLocaleString(numLocale)}
          </p>
          <p className="text-[10px] text-[#627D98]">{bi(isAr, "متوسط السوق", "Market average")}</p>
        </div>
        <div className="bg-[#FEF0EE] rounded-xl py-2 text-center">
          <p className="text-xs font-bold text-[#C0392B]">-{flag.diffPct}{isAr ? "٪" : "%"}</p>
          <p className="text-[10px] text-[#627D98]">{bi(isAr, "أقل من السوق", "Below market")}</p>
        </div>
      </div>
      ...
      <div className="bg-[#FEF0EE] rounded-xl px-3 py-2 mb-3 flex items-start gap-2">
        ...
        <p className="text-[10px] text-[#C0392B] leading-relaxed">
          {bi(isAr,
            ...
            "Price exceeds 100,000 OMR and is more than 30% below average. Requires compliance review.")}
        </p>
      </div>
      ...
      {isActionable && (
        <div className="flex flex-wrap gap-2">
          ...
          >
            {bi(isAr, "رفض الإعلان", "Reject listing")}
          </button>
        </div>
      )}
    </div>
