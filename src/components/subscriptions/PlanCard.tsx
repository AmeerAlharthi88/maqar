    ...
    <>
      ...
      >
        ...
        {isHighlighted && !isCurrent && (
          <span className="absolute -top-3 right-5 bg-[#C65D3B] text-white text-[10px] font-bold px-3 py-0.5 rounded-full">
            الأكثر طلباً
          </span>
        )}
        {isCurrent && (
          <span className="absolute -top-3 right-5 bg-[#5B8C5A] text-white text-[10px] font-bold px-3 py-0.5 rounded-full">
            خطتك الحالية
          </span>
        )}
        ...
        {/* Plan name & price */}
        <div className="mb-4">
          <p className="text-base font-bold text-[#1E1E1E] mb-2">{nameAr}</p>
          {isFree ? (
            <p className="text-3xl font-bold text-[#1E1E1E]">مجاني</p>
          ) : (
        ...
        <ul className="space-y-2.5 mb-5">
          {features.map((f, i) => (
            <li key={i} className="flex items-start gap-2">
              {f.included ? (
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#5B8C5A" strokeWidth="2.5" className="flex-shrink-0 mt-0.5" aria-label="متاح">
                  <polyline points="20 6 9 17 4 12" />
                </svg>
              ) : (
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#C4B5A5" strokeWidth="2.5" className="flex-shrink-0 mt-0.5" aria-label="غير متاح">
                  <line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" />
                </svg>
              )}
              <span className={cn("text-xs leading-relaxed", f.included ? "text-[#1E1E1E]" : "text-[#A89480]")}>
                {f.labelAr}
                {f.valueAr && (
                  <span className="font-semibold text-[#C65D3B]"> — {f.valueAr}</span>
                )}
              </span>
            </li>
          ))}
        ...
        {/* CTA */}
        {isCurrent ? (
          <p className="text-center text-xs text-[#5B8C5A] font-semibold py-2">
            خطتك النشطة
          </p>
        ) : isFree ? (
          <Link
            href="/auth/login"
            className="block w-full py-3 text-center rounded-xl bg-[#F5F0EA] text-[#7A6B5E] text-sm font-bold border border-[#E8DDD0]"
          ...
          <a
            ...
            className="block w-full py-3 text-center rounded-xl bg-[#5B8C5A] text-white text-sm font-bold"
          ...
          <>
            <button
              onClick={handleCTA}
              className="w-full py-3 rounded-xl bg-[#C65D3B] text-white text-sm font-bold"
              aria-label={`ترقية إلى ${nameAr}`}
            >
              ترقية إلى {nameAr}
            </button>
            {showLoginPrompt && (
              <div className="mt-3 bg-[#FBF0EB] rounded-xl p-3 text-center">
                <p className="text-xs text-[#C65D3B] font-semibold mb-1">
                  يلزم تسجيل الدخول
                </p>
                <Link
                  href="/auth/login"
                  className="text-xs text-[#C65D3B] underline"
                >
